...
function displayProduct(response) {
  ...
  boutton.addEventListener("click", (e) => {
    ...
    //Si quantité supérieure à 0 ET inférieure à 100 ET couleur sélectionnée ET nombre entier => on appelle la fonction addToCart.
    if (
      colorSelected != "" &&
      quantitySelected > 0 &&
      ...
      alert(
        "Vous devez sélectionner une quantité valide (nombre entier compris entre 1 et 100) et une couleur"
      );
    }
  });
}
...
function getCart() {
  ...
  if (cart === null || cart === undefined) {
    //s'il n'y a rien dans le localStorage, on récupère un array.
    return [];
  } else {
    //sinon on récupère un objet JSON du panier.
...
function addToCart(product) {
  ...
  ); // fonction find() pour vérifier, dans le tableau, la présence d'un élément.
